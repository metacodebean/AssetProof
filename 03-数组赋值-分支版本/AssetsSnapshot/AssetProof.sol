...
contract AssetProof is StateConst,DataStructure{
    ...
    function insertSingleSnapshot(address[] memory userAddress,uint256[] memory balances,uint256 totalAmount) public returns(bool success) {
        require(adminSetupInfo.assetSnapshotSyncAddress == msg.sender,"Only the account assetSnapshotSyncAddress can do this");
        require(snapshotState.syncState == ASSETS_SNAPSHOT_IS_SYNCING,"Synchronization has not yet started or has ended");
        require(userAddress.length == balances.length,"addresses and balances number are not match.");
        userSnapShots[snapshotState.tempCurrentSnapshotVersion].users = userAddress;
        userSnapShots[snapshotState.tempCurrentSnapshotVersion].balances = balances;
        snapshotState.syncCounter = uint32(userAddress.length);
        snapshotState.tempSnapshotTotalAssets = totalAmount;
        
        success = true;
    }
    
    
    function deleteSingleSnapshot() public returns(bool success) {
        require(adminSetupInfo.assetSnapshotSyncAddress == msg.sender,"Only the account assetSnapshotSyncAddress can do this.");
        require(snapshotState.syncState == ASSETS_SNAPSHOT_IS_SYNCING,"Synchronization has not yet started or has ended");
        require(userSnapShots[snapshotState.tempCurrentSnapshotVersion].users.length > 0,"The data to be deleted does not exist.");
        delete userSnapShots[snapshotState.tempCurrentSnapshotVersion];
        success = true;
    }
    ...
    function queryAssets(address userAddress,uint64 version) public view returns (uint256 balance,uint256 datetime) {
        ...
        address[] memory tempUserAddresses = userSnapShots[version].users;
        for(uint32 i=0;i<uint32(tempUserAddresses.length);i++){
            if(userAddress == tempUserAddresses[i]){
                balance = userSnapShots[version].balances[i];
    ...
    function reservesWithdraw(uint256 amount) public returns(bool success) {
     ...
     //Verify an address is in an address array.
    function addressIsInCurrentSnapshotAndHaveBalance(address oneAddress) internal returns(bool,uint256){
        for(uint i=0;i<userSnapShots[snapshotState.currentSnapshotVersion].users.length;i++){
            if(oneAddress == userSnapShots[snapshotState.currentSnapshotVersion].users[i]){
                if(userSnapShots[snapshotState.currentSnapshotVersion].balances[i]>0){
                   return (true,i);
                }
            }
        }
    ...
    function userApplyForWithdraw() public returns(bool success,uint64 currentId){
        bool isOk;
        uint addressIdx;
        (isOk,addressIdx) = addressIsInCurrentSnapshotAndHaveBalance(msg.sender);
        require(isOk,"The user is not in the current snapshot list");
        //wirte to order
        lastOrderId++;
        withdrawOrders[lastOrderId].orderId = lastOrderId;
        withdrawOrders[lastOrderId].withdrawAddress = msg.sender;
        withdrawOrders[lastOrderId].orderAmount = userSnapShots[snapshotState.currentSnapshotVersion].balances[addressIdx];
        ...
        userSnapShots[snapshotState.currentSnapshotVersion].withdrawFlags[msg.sender] = USER_WITHDRAW_SNAPSHOT_PENDING;
    ...
    function userCancelWithdrawOrder(uint64 orderId) public returns(bool success) {
        ...
        userSnapShots[snapshotState.currentSnapshotVersion].withdrawFlags[msg.sender] = USER_WITHDRAW_SNAPSHOT_NOTHING;
        success =  true;
    }
    ...
    function userWithdraw(uint64 orderId) public returns(bool success) {
        ...
        require(isOk,"The user is not in the current snapshot list");
        require(withdrawOrders[orderId].orderState == USER_WITHDRAW_ORDER_PENGING,"Incorrect order status.");
        require(userSnapShots[snapshotState.currentSnapshotVersion].balances[addressIdx] > 0,"The balance of this snapshot has been withdrawn.");
        ...
        userSnapShots[snapshotState.currentSnapshotVersion].balances[addressIdx] = 0;
        snapshotState.snapshotTotalAssets = snapshotState.snapshotTotalAssets.sub(withdrawOrders[orderId].orderAmount);
        userSnapShots[snapshotState.currentSnapshotVersion].withdrawFlags[msg.sender] == USER_WITHDRAW_SNAPSHOT_SUCCEED;
