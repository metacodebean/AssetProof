...
contract AssetProof is StateConst,DataStructure{
    ...
    function insertSingleSnapshot(address[] memory userAddresses,uint256[] memory balances) public returns(bool success) {
        require(adminSetupInfo.assetSnapshotSyncAddress == msg.sender,"Only the account assetSnapshotSyncAddress can do this");
        require(snapshotState.syncState == ASSETS_SNAPSHOT_IS_SYNCING,"Synchronization has not yet started or has ended");
        require(userAddresses.length == balances.length,"addresses and balances number are not match.");
        for(uint i=0;i<userAddresses.length;i++){
            if(!userSnapShots[userAddresses[i]][snapshotState.tempCurrentSnapshotVersion].isExist){
                userSnapShots[userAddresses[i]][snapshotState.tempCurrentSnapshotVersion].isExist = true;
                snapshotState.syncCounter++;
                snapshotState.tempSnapshotTotalAssets = snapshotState.tempSnapshotTotalAssets.add(balances[i]);
            }else{
        
                uint256 oldBalance = userSnapShots[userAddresses[i]][snapshotState.tempCurrentSnapshotVersion].balance;
                snapshotState.tempSnapshotTotalAssets = snapshotState.tempSnapshotTotalAssets.sub(oldBalance);
                snapshotState.tempSnapshotTotalAssets = snapshotState.tempSnapshotTotalAssets.add(balances[i]);
            }
            userSnapShots[userAddresses[i]][snapshotState.tempCurrentSnapshotVersion].balance = balances[i];
        }
    ...
    function addressIsInArrray(address oneAddress,address[] memory allAddresses) internal returns(bool){
        for(uint i=0;i<allAddresses.length;i++){
            if(oneAddress == allAddresses[i]){
                return true;
                break;
            }
        }
